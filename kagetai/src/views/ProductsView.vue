        ...
        <div class="columns" style="min-height: 100%">
                        ...
                        <li>
                            <div class="select">
                                <select v-model="selected_pt">
                                    ...
                                    <option v-for="item in products_type" :key="item.id" :value="item.product_type_id">
                                        {{ item.type_name }}
                                    </option>
                                </select>
                            </div>
            ...
            <div class="column is-8 pt-5">
                ...
                <div class="container is-fullhd mb-6" style="min-height: 600px">
                    <h1 class="title mb-6 mt-6 has-text-white" v-if="filteredProducts.length == 0">
                        ยังไม่มีสินค้า
                    </h1>
                    <div class="is-multiline columns" v-if="filteredProducts.length != 0">
                        <!-- card column -->
                        <div class="column is-one-third" v-for="product in filteredProducts" :key="product.product_id">
                            <div class="card">
...
export default {
    ...
    methods: {
        ...
        chatwith(seller_id) {
            ...
            axios
                ...
                .then((res) => {
                    ...
                    this.$router.push({ path: '/chat', query: { current_room_id: res.data.room_id } });
        ...
        addfav(proid, id) {
            console.log(proid)
            console.log(id)
            axios.post(`http://localhost:3000/favorite/${proid}/${id}`)
                .then((res) => {
                    console.log(res);
